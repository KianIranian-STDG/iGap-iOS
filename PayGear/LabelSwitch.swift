...
public class LabelSwitch: UIView {
    ...
    public var switchChangeType : SwitchChangeType!
    
    public var curState: SwitchState {
        didSet{
            switch curState {
            case .L://kart
                if (switchChangeType == .tap) {
                    updateUIState(leftUIState)
                }
                else {
                    updateUIStateOnDragging(leftUIState)
                }
                
            case .R://etebari
            if (switchChangeType == .tap) {
                updateUIState(rightUIState)
            }
            else {
                updateUIStateOnDragging(rightUIState)
                }
            }
        }
    }
    ...
    @IBInspectable public var labelsTextFont: UIFont = SMFonts.IranYekanRegular(10) {
        didSet {
            for label in [leftLabel, rightLabel] {
                label.font = labelsTextFont
            }
        }
    }
    ...
    private func setupBounds () {
        let circleSize = max(fontSize, fontSize)
        edge = circleSize * 0.2
    ...
    @objc func draggedView(_ sender:UIPanGestureRecognizer){
        switchChangeType = .drag
        self.bringSubviewToFront(circleView)
        let translation = sender.translation(in: self)
        if circleView.center.x + translation.x > 0 || circleView.center.x + translation.x < bounds.width {
            sender.view?.center = CGPoint(x: circleView.center.x + translation.x, y: circleView.center.y)
            sender.setTranslation(CGPoint.zero, in: self)
        }
        
        if circleView.center.x > bounds.width / 2 {
            if (curState == .L) {
                self.curState = .R
            }
        }
        ...
        if sender.state == .ended {
            ...
            if circleView.center.x > bounds.width / 2 {
                circleView.center = CGPoint(x: bounds.width - circleView.bounds.width / 2 - circlePadding, y: circleView.center.y)
                self.delegate?.switchChangToState(.R)
            }
            else {
                circleView.center = CGPoint(x: circleView.bounds.width / 2 + circlePadding, y: circleView.center.y)
    ...
    private func updateUIStateOnDragging(_ state: TextTypeUIState) {
        leftTextMask.frame        = state.leftTextMaskFrame
        rightTextMask.frame       = state.rightTextMaskFrame
