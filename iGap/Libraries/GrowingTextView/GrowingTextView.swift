...
open class GrowingTextView: UITextView {
    ...
    private var tfHeightConstraint: NSLayoutConstraint?
    ...
    override open func layoutSubviews() {
        ...
        if (tfHeightConstraint == nil) {
            tfHeightConstraint = NSLayoutConstraint(item: self, attribute: .height, relatedBy: .equal, toItem: nil, attribute: .notAnAttribute, multiplier: 1.0, constant: height)
            addConstraint(tfHeightConstraint!)
        }
        
        // Update height constraint if needed
        if height != tfHeightConstraint!.constant {
            shouldScrollAfterHeightChanged = true
            tfHeightConstraint!.constant = height
