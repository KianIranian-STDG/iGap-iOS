...
class IGClientGetRoomListRequest : IGRequest {
    ...
    class Handler : IGRequest.Handler{
        class func interpret(response responseProtoMessage:IGPClientGetRoomListResponse, removeDeleted: Bool = false) -> Int {
            ...
            IGFactory.shared.saveRoomsToDatabase(igpRooms, ignoreLastMessage: false, enableCache: true)
...
class IGClientGetRoomRequest : IGRequest {
    ...
    public static var roomIdArrayList : [Int64] = []
    
    class func sendRequestAvoidDuplicate(roomId: Int64, success: ((_ room: IGRoom) -> Void)? = nil){
    ...
    class func sendRequest(roomId: Int64){
        IGClientGetRoomRequest.Generator.generate(roomId: roomId).success({ (protoResponse) in
            ...
            if errorCode == .timeout {
                ...
                IGHelperAlert.shared.showCustomAlert(view: nil, alertType: .alert, title: IGStringsManager.GlobalWarning.rawValue.localized, showIconView: true, showDoneButton: false, showCancelButton: true, message: IGStringsManager.RoomNotExist.rawValue.localized, cancelText: IGStringsManager.GlobalClose.rawValue.localized)
