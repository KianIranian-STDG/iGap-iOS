...
class IGGlobal {
    ...
    /***** Base of Directories *****/
    static var showTypingBubble : Bool = false
    ...
    static var pickedImageThumbnail : UIImage!
...
extension UITapGestureRecognizer {
    ...
    func didTapAttributedTextInTextNode(node: ASTextNode, inRange targetRange: NSRange) -> Bool {
        // Create instances of NSLayoutManager, NSTextContainer and NSTextStorage
        let layoutManager = NSLayoutManager()
        let textContainer = NSTextContainer(size: CGSize.zero)
        let textStorage = NSTextStorage(attributedString: node.attributedText!)
        ...
        // Configure textContainer
        textContainer.lineFragmentPadding = 0.0
//        textContainer.lineBreakMode = node.line //node.lineBreakMode
        textContainer.maximumNumberOfLines = Int(node.lineCount)
        let labelSize = node.bounds.size
        textContainer.size = labelSize
        
        // Find the tapped character location and compare it to the specified range
        let locationOfTouchInLabel = self.location(in: node.view)
        let textBoundingBox = layoutManager.usedRect(for: textContainer)
...
//MARK: - Realm Object Detacher Extension
