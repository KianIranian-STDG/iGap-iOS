...
open class IGDropDown : UITextField{
    ...
    @IBInspectable public var rowBackgroundColor: UIColor = ThemeManager.currentTheme.ModalViewBackgroundColor
...
extension IGDropDown : UITextFieldDelegate {
...
extension IGDropDown: UITableViewDataSource {
    ...
    public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell?.textLabel?.font = UIFont.igFont(ofSize: 10)
        cell?.textLabel?.textAlignment = self.textAlignment
        cell?.textLabel?.numberOfLines = 0
        cell?.textLabel?.lineBreakMode = .byWordWrapping
        cell?.textLabel?.sizeToFit()
        cell?.textLabel?.textColor = ThemeManager.currentTheme.LabelColor
        return cell!
    }
}

extension IGDropDown: UITableViewDelegate {
