...
class IGNodeMusicPlayerTableViewController: UITableViewController {
    let cellID = "nodeMusicCell"
...
//        btnOrder.setTitle("🎛", for: .normal)
    ...
    @objc func buttonNextAction(_ sender:UIButton!) {
        ...
        if fileExist {
            ...
            IGNodePlayer.shared.startPlayer(roomMessage: currentPlayingItem,room: self.room,isfromBottomPlayer: true)
    ...
    @objc func sliderValueChanged(slider: UISlider, event: UIEvent) {
        if let touchEvent = event.allTouches?.first {
            ...
            case .moved:
                ...
                player.seekToTime(value: CMTimeMakeWithSeconds(Float64(((slider.value) * (IGGlobal.topBarSongTime / 100)) * 100), preferredTimescale: IGNodePlayer.shared.attachmentTimeScale))
...
//        btnShare.backgroundColor = .red
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if fileExist {
            SwiftEventBus.post(EventBusManager.stopLastButtonState)
            IGNodePlayer.shared.startPlayer(roomMessage: currentLastItem,room: self.room,isfromBottomPlayer: true)
...
extension IGNodeMusicPlayerTableViewController: PanModalPresentable {
