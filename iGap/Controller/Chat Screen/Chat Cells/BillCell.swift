...
class BillCell: IGMessageGeneralCollectionViewCell {
    ...
    class func nib() -> UINib {
        return UINib(nibName: "BillCell", bundle: Bundle(for: self))
    }
    ...
    func initChangeLang() {
        billMessage.text = IGStringsManager.BillMessage.rawValue.localized
    ...
    override func setMessage(_ message: IGRoomMessage, room: IGRoom, isIncommingMessage: Bool, shouldShowAvatar: Bool, messageSizes: MessageCalculatedSize, isPreviousMessageFromSameSender: Bool, isNextMessageFromSameSender: Bool) {
        ...
        guard let bill = message.wallet?.bill else {
            return
        }
        
        txtAmountValue.text = String(describing: bill.amount).inLocalizedLanguage() + " " + IGStringsManager.Currency.rawValue.localized
        txtBillTypeValue.text = bill.billType
        txtBillingIdValue.text = bill.billId?.inLocalizedLanguage()
        txtPaymentCodeValue.text = bill.payId?.inLocalizedLanguage()
        txtCardNumberValue.text = bill.cardNumber?.inLocalizedLanguage()
        txtOrderIdValue.text = String(describing: bill.orderId).inLocalizedLanguage()
        txtTerminalIdValue.text = String(describing: bill.terminalNo).inLocalizedLanguage()
        txtReferenceIdValue.text = String(describing: bill.rrn).inLocalizedLanguage()
        txtTrackingCodeValue.text = String(describing: bill.traceNumber).inLocalizedLanguage()
        
        if let time = TimeInterval(exactly: bill.requestTime) {
