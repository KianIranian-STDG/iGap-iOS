    ...
    func requestToAddAdminInChannel() {
        ...
        for member in selectedUsers {
            if let channelRoom = room {
                ...
                IGChannelAddAdminRequest.Generator.generate(roomID: channelRoom.id, memberID: member.registredUser.id, adminRights: IGPChannelAddAdmin.IGPAdminRights()).success({ [weak self] (protoResponse) in
                    IGGlobal.prgHide()
    ...
    func requestToAddAdminInGroup() {
        ...
        for member in selectedUsers {
            if let groupRoom = room {
                IGGlobal.prgShow(self.view)
                IGGroupAddAdminRequest.Generator.generate(roomID: groupRoom.id, memberID: member.registredUser.id, adminRights: IGPGroupAddAdmin.IGPAdminRights()).success({ [weak self] (protoResponse) in
                    IGGlobal.prgHide()
                    DispatchQueue.main.async {
                        self?.manageClosePage()
